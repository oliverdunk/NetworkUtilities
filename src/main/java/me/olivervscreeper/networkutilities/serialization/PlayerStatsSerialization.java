 ...
 * This serialization class supports optional serialization.<br/> TacoSerialization will create a
 * folder in your server plugins directory (wherever that may be) called 'TacoSerialization'. Inside
 * the folder will be a config.yml file. Various values can be turned off to prevent some keys from
 * being generated.
 *
 * @author KILL3RTACO
  ...
  public static JSONObject serializePlayerStats(Player player) {
    try {
      ...
      root.put("potion-effects",
               PotionEffectSerialization.serializeEffects(player.getActivePotionEffects()));
