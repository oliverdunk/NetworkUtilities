 ...
 * it is cast as such and redirected to PlayerSerialization. <br/> <br/> This serialization class
 * supports optional serialization.<br/> TacoSerialization will create a folder in your server
 * plugins directory (wherever that may be) called 'TacoSerialization'. Inside the folder will be a
 * config.yml file. Various values can be turned off to prevent some keys from being generated.
 *
 * @author KILL3RTACO
...
//        }
    try {
      ...
      root.put("potion-effects",
               PotionEffectSerialization.serializeEffects(entity.getActivePotionEffects()));
  ...
  /**
   * Spawn a LivingEntity in a desired Location with the given stats. The given string will be
   * constructed into a JSONObject to use a reference to the entities stats.
   *
      ...
      if (!stats.has("type")) {
        ...
        LivingEntity
            ...
            (LivingEntity) location.getWorld()
                .spawnEntity(location, EntityType.fromId(stats.getInt("type")));
